...
9. **Persistencia y reutilizacion de datos historicos**: los klines descargados (`klines.db`, cache Parquet en `reports/cache/parquet/` y artefactos en `reports/entregables/datasets/`) **no deben borrarse ni ignorarse** en corridas futuras. Toda descarga es acumulativa: reutilizar la base existente, completar solo ventanas faltantes (ZIP mensual + API para mes parcial) y registrar el dataset con `dataset prepare` + `dataset verify`. No sobrescribir ni descartar series ya materializadas salvo corrupcion demostrada; preferir `--overwrite_cache` solo cuando el operador confirme regeneracion. Los scripts y corridas deben apuntar a la misma `klines.db` compartida del repo.
10. **Rutina de resolucion de simbolos Alpha (Binance Alpha market)**: antes de descargar klines o lanzar un estudio sobre cualquier simbolo del mercado Alpha, **siempre** ejecutar las validaciones de `BinanceDownloader.resolve_alpha_symbol`: (a) token-list lookup, (b) de-duplicacion por alphaId cuando el mismo human symbol existe en varios chains (preferir `not offline > not offsell > mayor liquidez > mayor volume24h` y logguear alternativas descartadas), (c) cross-check con `get-exchange-info` para detectar el quote tradeable real (algunos Alpha solo cotizan USDT, otros USDC), (d) persistir el simbolo humano normalizado (`PHAROS` -> `PHAROSUSDC`) en `klines.db` y en `alpha_token.json`. Estas validaciones estan testeadas en `tests/test_alpha_resolver.py`. Cualquier nueva regla arbitraria de Alpha debe añadirse al codigo, al test de regresion, y al modelo canonico en [`library/bots/agartha/ALPHA_STUDY_MODEL.md`](library/bots/agartha/ALPHA_STUDY_MODEL.md).
